...
export default function InitialMutableBookList({ createOrUpdateBook, removeBook, listBooks}: Props) {
    ...
    return (<>
        <form id="add_book_form" class="flex flex-col justify-items-center dark:bg-slate-800 bg-slate-200 p-10 col-start-1 col-span-1 gap-1">
          ...
          <input class='text-black' name='author' placeholder='Author' value={new_book().author} onInput={(e) => update_new_book(e, "author")}/>
          
          <label for='image'>Image URL</label>
          <input class='text-black' name='image' placeholder='Image URL' value={new_book().image} onInput={(e) => update_new_book(e, "image")}/>
          
          <label for='price'>Price</label>
          <input class='text-black' name='price' type="number" placeholder='0.0' value={new_book().price} onInput={(e) => update_new_book(e, "price")}/>
        ...
        <ul class="grid col-start-2 p-10 grid-cols-[auto_1fr_auto_min-content] grid-rows-[repeat(auto-fit,_minmax(10px,_auto)_minmax(10px,_auto)_minmax(10px,_1fr))] gap-y-1 gap-x-3">
          <Key each={books()} by="id">
            ...
            <li x-book={item().id} x-book-name={item().name} class="book grid grid-cols-subgrid grid-rows-subgrid col-span-4 row-span-3">
              ...
              <h2 class=" col-start-2 col-span-1 row-start-1">Name: <input x-name="name" class="text-black" value={item().name} onInput={(e) => update_existing_book(e, "name", index())}/></h2>
              <h3 class="col-span-1 row-start-1 col-start-3">Price: <input x-name="price" class="text-black" value={item().price} type="number" onInput={(e) => update_existing_book(e, "price", index())}/></h3>
              <h4 class="col-span-1 col-start-2 row-start-2">Author: <input x-name="author" class="text-black" value={item().author} onInput={(e) => update_existing_book(e, "author", index())}/></h4>
              <h4 class="col-span-1 col-start-3 row-start-2">Image URL: <input x-name="image" class="text-black" value={item().image} onInput={(e) => update_existing_book(e, "image", index())}/></h4>
              <div class="col-span-3 col-start-2 flex flex-row row-start-3">Description: <textarea x-name="description" class="text-black flex-grow" value={item().description} onInput={(e) => update_existing_book(e, "description", index())}/></div>
