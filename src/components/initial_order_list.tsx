...
export default function InitialOrderList({ lookupBookById, listOrders, findBookOnShelf, fulfilOrder }: Props) {
    let [orders, { refetch }] = createResource(async () => {
        ...
        let orders_with_book_info = await Promise.all(orders.map(async ({ orderId, books }) => {
            let books_collected: Record<string, number> = {};
            for (const book of books) {
                if (book in books_collected) {
                    books_collected[book] += 1;
                } else {
                    books_collected[book] = 1;
                }
            }
            let book_info_promise = Promise.all(Object.keys(books_collected).map((bookId) => lookupBookById(bookId)));
            let book_position_promise = Promise.all(books.map((bookId) => findBookOnShelf(bookId)));
            ...
            for (let i = 0; i < book_info.length && i < book_position.length; i++) {
                books_in_order[i] = {
                    ...book_info[i],
                    copies: books_collected[book_info[i].id || ""] || 0,
